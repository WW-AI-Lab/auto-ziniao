...
- CLI：`commander`，生产命令为 `ziniao ...`（或 `pnpm ziniao ...`）。
...
2. 跑通后执行 `pnpm ziniao new <flow_id> <中文名>` 创建脚手架。
...
4. 验收：`pnpm ziniao validate <flow_id>` 通过 → `pnpm ziniao run <flow_id> -v --no-heal` 真机跑通 → 检查 `data/output/` 产出正确。
...
4. 验证：`pnpm ziniao validate <id>` → `pnpm ziniao run <id> -v --no-heal` 必须真实跑通。
5. 固化：把修复写入 `learnings/known_issues.json`（pattern 取错误信息中稳定子串，`resolved: true`）。
6. 通知用户：根因、修复内容、验证结果。

### 模式 C：日常操作（执行/排查）

- 执行：`pnpm ziniao run <flow> [-p k=v]...`；重试：`pnpm ziniao retry <flow>`。
- 排查：`pnpm ziniao history` / `pnpm ziniao heals` / `pnpm ziniao stats`；bridge 日志用 `get_logs` 工具。
...
| 路径 | 作用 | 修改约束 |
|------|------|----------|
| `packages/cli/` | 生产 `ziniao` CLI 入口 | 不得直接访问 bridge；命令编排只能 |
