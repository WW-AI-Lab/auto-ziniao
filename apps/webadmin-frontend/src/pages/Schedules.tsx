...
type ScheduleTriggerView = { type: string; minutes?: number; time?: string; expr?: string }

function triggerLabel(t: Schedule['trigger']): string {
  const trigger = t as ScheduleTriggerView
  if (trigger.type === 'interval') return `每 ${trigger.minutes} 分钟`
  if (trigger.type === 'daily') return `每日 ${trigger.time}`
...
export default function Schedules() {
  ...
  const openEdit = (target: Schedule | 'new') => {
    ...
    if (target === 'new') {
      ...
      const trigger = target.trigger as ScheduleTriggerView
      form.setFieldsValue({
        name: target.name,
        flow_id: target.flow_id,
        trigger_type: trigger.type as FormValues['trigger_type'],
        minutes: trigger.minutes,
        time: trigger.time ? dayjs(trigger.time, 'HH:mm') : undefined,
        expr: trigger.expr,
