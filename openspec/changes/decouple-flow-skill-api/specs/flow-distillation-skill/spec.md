...
`ziniao-assistant` 的 flow 沉淀工作流 SHALL 通过 WebAdmin API 完成 flow 创建、extract 保存、flow 校验、flow 运行和产出确认。该 workflow MUST NOT 要求 Agent 读取或写入 repo-local `flows/`、`extracts/`、`docs/`、`data/` 路径，MUST NOT 要求执行 `pnpm ziniao ...` CLI 命令作为沉淀步骤。
...
- **THEN** 文档不要求读取 `$REPO/docs/03-流程定义规范.md`、`$REPO/flows/_template.json`、`$REPO/flows/*.json`，也不要求执行 `pnpm ziniao new`、`pnpm ziniao validate` 或 `pnpm ziniao run`
